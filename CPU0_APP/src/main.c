...
int main()
{
	...
	while(1)
	{
		if (key_flag)
		{
			 ...
			 * Write led value
			 */
			XGpioPs_WritePin(&GPIO_PTR, 7, LedVal) ;
			LedVal = ~LedVal ;
			/*
...
int PsGpioSetup(XScuGic *InstancePtr)
{
	...
	/* set MIO 7 as output */
	XGpioPs_SetDirectionPin(&GPIO_PTR, 7, GPIO_OUTPUT) ;
	/* enable MIO 7 output */
	XGpioPs_SetOutputEnablePin(&GPIO_PTR, 7, GPIO_OUTPUT) ;
	/* set interrupt type */
...
int GpioHandler(void *CallbackRef)
{
	...
	Int_val = XGpioPs_IntrGetStatusPin(GpioInstancePtr, 51) ;
	/* clear key interrupt */
